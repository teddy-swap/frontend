...
import SearchFieldContained from "../components/SearchFieldContained";
...
export default function Orders() {
  ...
  const actions = ["all", "swap", "deposit", "withdraw"];
  ...
  return (
    <Box className="dark:text-white w-[1200px] z-0 dark:bg-zinc-900 rounded-2xl">
      <Box className="space-y-6 px-6 py-8">
        ...
        <Stack className="grid grid-cols-3 gap-6">
          <Stack className="gap-2 flex-grow">
            <InputLabel className="text-sm font-semibold dark:text-white">
              Search by Token
            </InputLabel>
            <SearchFieldContained placeholder="Search by token name or policy ID" />
          </Stack>
          <Stack className="gap-2 flex-grow">
            <InputLabel className="text-sm font-semibold dark:text-white">
              Search by action
            ...
            <Select
              ...
              variant="outlined"
              displayEmpty
              className="w-full dark:text-white border dark:border-cyan-900 rounded-lg capitalize dark:bg-cyan-900/10"
              classes={{
              ...
              MenuProps={{
                PopoverClasses: {
                  paper: darkMode ? "bg-[#181e22]" : "",
                },
              }}
            >
              {actions.map((action) => (
                <MenuItem
                  className="capitalize hover:bg-cyan-900/50 transition text-white"
                  key={action}
                  value={action}
                >
                  {action}
